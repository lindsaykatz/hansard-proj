...
**To enhance the range of research questions which can be explored with our data, we have created a supplementary file containing the debate topics and their corresponding page numbers for each sitting day in our database. To extract these data, we wrote a script to parse all debate information and sub-debate information elements from each XML file in chronological order, and add a date variable with the date of each sitting day. This left us with a single dataframe containing the date, an item index variable to specify the order of proceedings (i.e. the order in which these topics were discussed), a title variable containing the debate or sub-debate title, and a page number variable specifying on which page that title can be found in the official Hansard PDF.**

```{r, eval=F}
# read in df
topics <- arrow::read_parquet("../additional_data/all_debate_topics.parquet")

# grab first 5 titles from some sitting day
topics |> filter(date == "2015-03-16") |> slice(1:5) |> kableExtra::kable()
```

# Data Records

Our database is available in both CSV and parquet formats. It covers all sitting days of the House of Representatives from 02 March 1998 to 08 September 2022, so there are 1,517 files for each format. **Additionally, there is a single corpus Parquet file containing the data from all sitting days, with a date variable added to allow for distinction and filtering of individual sitting days.** All data records are available on the general-purpose repository Zenodo, at https://doi.org/10.5281/zenodo.7799678 [@katz_2023]. For each file, each row contains an individual statement, with details on the individual speaking. For general statements transcribed as made by "Honourable members" for example, these variables cannot be specified. @tbl-vars provides an overview of each variable found in the database.
