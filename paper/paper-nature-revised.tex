...
as judged by the Speaker determines the outcome\textsuperscript{3}.
Further, Members can vote in an unofficial arrangement called pairs, in
which as written in Chapter 8 of\textsuperscript{3}, ``can be used to
enable a Member on one side of the House to be absent for any votes when
a Member from the other side is to be absent at the same time or when,
by agreement, a Member abstains from voting''. If the result as
determined by the Speaker is challenged by more than one MP, this leads
to a division of the House in which the question is re-stated and
Members must move to the left or right of their chair depending on how
they vote, so that the votes can be re-counted and
recorded\textsuperscript{3}.}
