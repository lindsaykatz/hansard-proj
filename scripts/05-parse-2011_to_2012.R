...
split_interjections_maincomm <- function(main, interject, filename){
  ...
  # remove business start for now and we can add back in later
  #main_bus_start <- main %>% filter(name=="business start")
  #main <- main %>% filter(name!="business start")
  ...
  # add name we missed splitting on b/c didn't have a prefix - saw on 2011 09 12.
  if (filename=="2011-09-12.xml") {
    names_use <- c(names_use, "WYATT ROY")
  }
  ...
  # now separate rows
  main <- separate_rows(main, body, sep=paste0("(?=", interject_general, ")(?!", interject_general, "[[:space:]]on[[:space:]]my)(?!", interject_general, "[[:space:]]having)(?!", interject_general, "[[:space:]]standing)(?!", interject_general, "[[:space:]]will )(?!", interject_general, "[[:space:]]should )(?!", interject_general, "'s )(?!", interject_general, ", )",
                                               collapse="|"))
  ...
  if (length(other_names)>0){
    ...
    main <- main %>% mutate(name = ifelse(q_in_writing==0 & name!="business start" & grepl(paste0("^", c(interject_names, names_use, other_names), collapse = "|"), body), NA, name),
                            name.id = ifelse(q_in_writing==0 & name!="business start" & grepl(paste0("^", c(interject_names, names_use, other_names), collapse = "|"), body), NA, name.id),
                            electorate = ifelse(q_in_writing==0 & name!="business start" & grepl(paste0("^", c(interject_names, names_use, other_names), collapse = "|"), body), NA, electorate),
                            party = as.factor(ifelse(q_in_writing==0 & name!="business start" & grepl(paste0("^", c(interject_names, names_use, other_names), collapse = "|"), body), NA, as.character(party))))
  ...
  # transcription error where business start for maincomm was transcribed twice, once in the chamber.xscript node - let's remove that one
  if (filename=="2011-08-24.xml"){
    main <- main %>% filter(!(fedchamb_flag==0 & str_detect(body, "\\(Hon\\. Peter Slipper\\) took the chair|Order! There being no quorum present, the sitting of"))) %>% 
      mutate(name = ifelse(fedchamb_flag==1 & name=="business start" & str_detect(body, "^The DEPUTY SPEAKER:  Order!"),
                           "The DEPUTY SPEAKER", name),
             body = ifelse(fedchamb_flag==1 & str_detect(body, "^The DEPUTY SPEAKER:  Order!"),
  ...
  main <- separate_rows(main, body, sep=paste0("(?=Proceedings suspended from \\d\\.\\d\\d to \\d\\d\\.\\d\\d)"))
  ...
  if (filename=="2012-05-31.xml"){
    main <- separate_rows(main, body, sep=paste0("(?<=\\(Time expired\\) )(?=I seek leave to incorporate these documents in support of my speech\\.)"))
  }
  
  if (filename=="2012-02-07.xml"){
    main <- separate_rows(main, body, sep=paste0("(?<=\\(Time expired\\))(?=On indulgence, I table a petition by those who wish to save the Highett Grassy Woodland\\.)"))
  }
  ...
  main <- separate_rows(main, body, sep=paste0("(?<=\\(Time expired\\))(?=Question put\\: That the motion \\(.{1,20}\\) be agreed to\\.)"))
  main <- separate_rows(main, body, sep=paste0("(?<=\\(Time expired\\))(?=Question put\\: That the amendment \\(.{1,20}\\) be agreed to\\.)"))
  main <- separate_rows(main, body, sep=paste0("(?<=\\(Time expired\\))(?=The petition read as follows-)"))
  main <- separate_rows(main, body, sep=paste0("(?<=\\(Time expired\\))(?=Petition received\\.)"))
  ...
  stage_notes <- c("Bill read a [[:alpha:]]{0,10}[[:space:]]time\\.",
                   ...
                   "\\(Quorum formed\\)",
                   "Proceedings suspended from \\d\\d\\:\\d\\d to \\d\\d\\:\\d\\d",
                   "Proceedings suspended from \\d\\.\\d\\d to \\d\\d\\.\\d\\d",
                   "Question put\\: That the motion \\(.{1,20}\\) be agreed to\\.",
                   "Question put\\: That the amendment \\(.{1,20}\\) be agreed to\\.",
  ...
  if (nrow(main %>% filter(name=="business start"))==0){
    ...
    main <- main %>% filter(!(name!="business start" & str_detect(body, "took the chair at \\d\\d:\\d\\d|took the chair at \\d:\\d\\d")))
  }
  ...
  #     mutate(speech_no = ifelse(name=="business start", 0, speech_no)) %>%
  ...
  master_list <- all %>% filter(deathDate >= str_remove(filename, ".xml") | is.na(deathDate)) %>% 
...
parse_hansard_new <- function(filename) {
  ...
  if (nrow(xmlToDataFrame(node=getNodeSet(hansard_xml, "//chamber.xscript/debate/subdebate.1/question/talk.start/talker"))) > 0) {
    ...
    q_wo_notice <- q_wo_notice %>% mutate(question = ifelse(talk.text %in% q_wo_notice_text, 1, 0),
      ...
      # adding march 23 based on error caught in 2012-02-13
  ...
  # doing it this way with process of elimination b/c usually subdebate 1 has the most content
  # first, debate text
  if (nrow(tibble(xmlToDataFrame(node=getNodeSet(hansard_xml, "//maincomm.xscript/debate/speech/talk.text")))) > 0) {
    debate_text_fed <- item_df(xml_df, "//maincomm.xscript/debate/speech/talk.text") %>%
      select(body) %>%
      unnest(body) %>%
      mutate(sub1_flag_use = 0, sub2_flag_use = 0) %>% 
      mutate(body = {if("body" %in% names(.))  ifelse(str_detect(body, "[[:lower:]][[:lower:]]\\.[[:upper:]]"),
  ...
  if (nrow(tibble(xmlToDataFrame(node=getNodeSet(hansard_xml, "//maincomm.xscript/debate//subdebate.2/speech/talk.text")))) > 0) {
    sub2_text_fed <- item_df(xml_df, "//maincomm.xscript/debate//subdebate.2/speech/talk.text") %>%
      select(body) %>%
      unnest(body) %>%
      mutate(sub1_flag_use = 0, sub2_flag_use = 1) %>% 
      mutate(body = {if("body" %in% names(.))  ifelse(str_detect(body, "[[:lower:]][[:lower:]]\\.[[:upper:]]"),
  ...
  main <- split_interjections_maincomm(main, interject, filename)
...
for(i in 1:length(files_get_maincomm)){
  parse_hansard_new(files_get_maincomm[i])
}
