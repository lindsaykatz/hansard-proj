...
# read in corpus
corpus <- read_parquet("/Volumes/Verbatim/v4/hansard-corpus/hansard_corpus_1998_to_2022.parquet")
...
```{r}
### need to confirm party abbreviation mapping!!!!!
s_by_party_name <- corpus |> select(date, speech_no, party) |> 
  filter(!is.na(party)) |> 
  ...
  summarise(total_distinct_speeches = sum(n_distinct_speeches)) |> 
  select(party_name_auspol, everything()) |> 
  arrange(desc(total_distinct_speeches)) |> 
  filter(!is.na(party_abb_auspol)) |> 
  rename("Party Name" = party_name_auspol,
         "Party Abbreviation" = party_abb_auspol,
         "Total # of Speeches" = total_distinct_speeches)

write_csv(s_by_party_name, "../paper/s_by_party_name.csv")
```

```{r}
parties_auspol <- AustralianPoliticians::get_auspol('allbyparty')

parties_auspol |> select(partyAbbrev, partyName) |> distinct()
```
...
# plot
s_by_day |> filter(n_distinct_speeches!=0) |> 
  ggplot(aes(x=as.Date(date), y=n_distinct_speeches, color=fedchamb_flag, fill=fedchamb_flag)) + 
  geom_point(alpha=0.5) +
  geom_smooth() +
  #facet_wrap(~fedchamb_flag, labeller = as_labeller(c(`0` = "Chamber", `1` = "Federation Chamber"))) +
  labs(x = "Date", y = "Number of speeches", color=NULL, fill=NULL) +
  theme_bw() +
  scale_color_discrete(labels=c("Chamber", "Federation Chamber")) +
  scale_fill_discrete(labels=c("Chamber", "Federation Chamber"))

ggsave("../paper/n_daily_speeches.png", width=8, height=5)
```
...
Let's compare to the numbers [here](https://www.aph.gov.au/-/media/02_Parliamentary_Business/22_Chamber_Documents/224_Statistics/House_of_Representatives/Statistics_Historical/sittings_statistics.pdf?la=en&hash=5B5E7E9518C4D7A416D0846FE0D44E063A4AB3FD).

```{r}
sitting_stats <- read_csv("~/Desktop/RA/sitting_stats.txt", col_names = FALSE)

# rename select vars based on columns in pdf linked above
...
# remove 2022 b/c we don't have complete year's data
meeting_per_year |> mutate(fedchamb_flag = case_when(fedchamb_flag == 0 ~"chamb",
  ...
  arrange(desc(diff))
```
I was able to manually check the discrepancies for the Chamber sittings, and found that there are two days in 1998 (May 6th and 7th) and two days in 2004 (February 10th and May 12th) where the House sat in the Chamber, but there is no XML transcript for Hansard available. The Federation Chamber discrepancies may be due to transcription errors - that is, Federation Chamber proceedings being nested in the Chamber proceedings node in the XML.
