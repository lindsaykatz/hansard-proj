...
parse_hansard_1998 <- function(filename){ 
  ...
  interject_specific <- str_extract_all(main$body, "(?<=\\.|\\. )(Mrs|Ms|Mr|Dr) .{1,20} interjecting$|(?<=\\.|\\. )(Mrs|Ms|Mr|Dr) .{1,20} interjecting—") %>% unlist() %>% na.omit() %>% unique()
  
  if (filename=="1999-02-17.xml") {
    interject_specific <- c(interject_specific, "Mr Swan interjecting—")
  }
  
  if (filename=="1998-12-02.xml" | filename=="1999-03-25.xml") {
    interject_specific <- c(interject_specific, "Mr Crean interjecting—")
  }
  
  if (filename=="1999-05-12.xml") {
    interject_specific <- c(interject_specific, "Mr Hockey interjecting—")
  }
  
  if (filename=="1999-10-18.xml") {
    interject_specific <- c(interject_specific, "Mr O'Connor interjecting—")
  }
  
  if (filename=="1999-11-25.xml") {
    interject_specific <- c(interject_specific, "Mr Snowdon interjecting—")
  }
  ...
  lookup <- left_join(lookup, master_list, by=c("last_name", "displayName", "title"), multiple = "all") %>% 
  ...
  # export CSV
  write.csv(main, paste0("/Volumes/Verbatim/output/main-1998-1999-v2/", str_remove(filename, ".xml"), "-main.csv"), row.names = FALSE)
