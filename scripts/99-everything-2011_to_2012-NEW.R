...
split_interjections_maincomm <- function(main, interject, bus_start, q_in_writing, filename){
  ...
  main <- bind_rows(main, q_in_writing)
  ...
  gender_uniqueID_list <- main %>% filter(is.na(gender) & is.na(uniqueID) & name!="business start" & name!="stage direction" 
    ...
    left_join(., master_list, by=c("displayName"), multiple = "all") %>% # last argument is to suppress new dplyr warning
...
parse_hansard_maincomm <- function(filename){
  ...
  main <- main %>% arrange(itemindex, fedchamb_flag, page.no, time.stamp) %>% 
    select(-itemindex, -span) %>% 
    mutate(q_in_writing = 0)
  
  # make sure number of rows hasn't changed
  stopifnot(nrow(main) == nrow(speech_order))
  ...
  main <- split_interjections_maincomm(main, interject, bus_start, q_in_writing, filename)
  ...
  write.csv(main, paste0("/Volumes/Verbatim/output/main-2011-2022-NEW-v2/", str_remove(filename, ".xml"), "-main.csv"), row.names = FALSE)
}
