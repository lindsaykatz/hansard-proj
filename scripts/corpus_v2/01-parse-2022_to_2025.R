...
split_interjections_fedchamb <- function(main, interject, filename){
  ...
  main <- separate_rows(main, body, sep=paste0("(?<=\\.|\\. )(?=Question agreed to, honourable members standing in their places\\.)"))
  ...
  main <- separate_rows(main, body, sep=paste0("(?<=\\(Time expired\\))(?=On that note, Mr Speaker, I ask that further questions be placed on the Notice Paper\\.)"))
  ...
  repeated_surnames <- master_list %>% group_by(title, last_name) %>% 
    ...
    add_row(title = "Mr", last_name = "Kennedy")
...
parse_hansard_new <- function(filename) {
  ...
  if (nrow(xmlToDataFrame(node=getNodeSet(hansard_xml, "//chamber.xscript/debate/subdebate.1/question/talk.start/talker"))) > 0) {
    ...
    if (filename %in% c("2020-10-19.xml", "2014-05-26.xml", "2012-08-15.xml", "2013-06-26.xml", "2013-11-13.xml" , "2016-09-13.xml", "2018-03-26.xml", "2023-10-19.xml")) {
  ...
  write.csv(main, paste0("/Volumes/Verbatim/output-2022_2025/", str_remove(filename, ".xml"), "-main.csv"), row.names = FALSE)
...
files_all <- list.files("/Volumes/Verbatim/input-2022_2025/")

for (i in 1:length(files_all)) {
  parse_hansard_new(files_all[i])
}


for (i in 1:length(dates_to_parse)) {
  parse_hansard_new(dates_to_parse[i])
}
