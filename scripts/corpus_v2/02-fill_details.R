...
fill_main <- function(filename) {
  ...
  # read in file
    thisFile <- read_csv(paste0("/Volumes/Verbatim/output-2022_2025/", filename), show_col_types = F)
  ...
  if (length(repeat_short_names)>0){
    ...
    if (length(repeat_full_names)>0){
      ...
      # issue specific to this day - need to go back into parsing script to figure out the source of the error
      if (filename=="2024-03-26-main.csv") {
        thisFile <- thisFile %>% 
          mutate(name = case_when(name=="Marino, Andrew" & body=="Mrs Marino interjecting-" ~ "Marino, Nola",
                                  name=="Burnell, Andrew" & body=="Mr Burnell interjecting-" ~ "Burnell, Matt",
                                  .default = name),
                 name.id = case_when(name %in% c("Marino, Nola", "Burnell, Matt") ~ NA,
                                     .default = name.id))
      }
  ...
  write.csv(thisFile, paste0("/Volumes/Verbatim/output_filled-2022_2025/", str_remove(filename, "-main.csv"), "-main-v2.csv"), row.names = FALSE)

}


# grab list of all file names
files_2022_2025 <- list.files("/Volumes/Verbatim/output-2022_2025")

for(i in 1:length(files_2022_2025)){
  fill_main(files_2022_2025[i])
  Sys.sleep(3)
}
