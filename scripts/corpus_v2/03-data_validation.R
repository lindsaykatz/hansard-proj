...
files_all <- list.files("/Volumes/Verbatim/output_filled-2022_2025")
...
session_headers <- read_csv("/Volumes/Verbatim/session_info_2022-2025.csv", show_col_types = F) %>% pull(date)
...
for(i in 1:length(files_all)) {
  # read in file
  thisFile <- read_csv(paste0("/Volumes/Verbatim/output_filled-2022_2025/", files_all[i]), show_col_types = F)
...
for (i in 1:length(test4_dates)) {
  ...
  # read in file
  thisFile <- read_csv(paste0("/Volumes/Verbatim/output_filled-2022_2025/", filename), show_col_types = F)
  ...
  # export file
  write.csv(thisFile_fixed, paste0("/Volumes/Verbatim/output_filled-2022_2025/", test4_dates[i]), row.names = F) 
...
for (i in 1:length(test5_dates)){
  # read in file
  thisFile <- read_csv(paste0("/Volumes/Verbatim/output_filled-2022_2025/", test5_dates[i]), show_col_types = F) 
  ...
  # export file
  write.csv(thisFile, paste0("/Volumes/Verbatim/output_filled-2022_2025/", test5_dates[i]), row.names = F) 
...
# just one issue: Mr Kennedy - Simon Kennedy, who joined parliament in 2024. Code is thinking it is David Kennedy.
# manually fixing this here
# grab list of dates where we need to fix the CSVs
test7_dates <- test7 %>% select(date) %>% unique() %>% mutate(date = paste0(date, "-main-v2.csv")) %>% pull(date)

# loop over those dates we just grabbed to fix name ID issues
for (i in 1:length(test7_dates)){
  # read in file
  thisFile <- read_csv(paste0("/Volumes/Verbatim/output_filled-2022_2025/", test7_dates[i]), show_col_types = F) 
  
  # fix uniqueID
  thisFile <- thisFile %>% 
    mutate(name = case_when(uniqueID=="Kennedy1940" ~ "Kennedy, Simon",
                            .default=name),
           name.id = case_when(uniqueID=="Kennedy1940" ~ "267506",
                               .default = name.id),
           uniqueID = case_when(uniqueID=="Kennedy1940" ~ NA,
                                .default=uniqueID))
  
  # export file
  write.csv(thisFile, paste0("/Volumes/Verbatim/output_filled-2022_2025/", test7_dates[i]), row.names = F) 
