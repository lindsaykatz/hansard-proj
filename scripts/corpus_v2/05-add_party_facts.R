...
corpus <- read_parquet(here("/Volumes/Verbatim/hansard-corpus/hansard_corpus_1998_to_2022.parquet"))
...
party_table_final <- read_csv("additional_data/lookup_tables/PartyFacts_map.csv", show_col_types = F)

# grab list of filenames to iterate over
all_csvs <- list.files("/Volumes/Verbatim/output_filled-2022_2025")
...
for (i in 1:length(all_csvs)){
  ...
  thisFile <- readr::read_csv(paste0("/Volumes/Verbatim/output_filled-2022_2025/", all_csvs[i]), 
  ...
  # export updated file as Parquet
  arrow::write_parquet(thisFile_new, paste0("/Volumes/Verbatim/hansard-daily-parquet-2022_2025/", str_remove(all_csvs[i], "csv"), "parquet"))
