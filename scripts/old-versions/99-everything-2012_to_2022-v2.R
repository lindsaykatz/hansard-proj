...
split_interjections <- function(main, interject, bus_start, q_in_writing, filename){
...
parse_hansard_fedchamb <- function(filename){
  ...
  main <- split_interjections(main, interject, bus_start, q_in_writing, filename)
  ...
  write.csv(main, paste0("/Volumes/Verbatim/output/main-2011-2022-NEW/", str_remove(filename, ".xml"), "-main.csv"), row.names = FALSE)
}

# grab list of all file names
files_all <- list.files("/Volumes/Verbatim/input/")

files_now <- test2 %>% select(date) %>% mutate(date = paste0(date, ".xml")) %>% filter(date >= "2012-08-14.xml") %>% pull()

# grab a couple years
files_get <- files_all %>%
  as_tibble() %>%
  filter(str_detect(value, "^2012-|^2013-|^2014-|^2015-|^2016-|^2017-|^2018-|^2019-|^2020-|^2021-|^2022-")) %>%
  filter(value >= "2012-08-14.xml") %>%
  pull(value)
